...
class Soccer {
    ...
    checkIfMoveIsValid = (xb: number, yb: number): boolean => {
        // check if at least one of delta values equals 1
        const deltaX = Math.abs(this.pos.x - xb) === 1
        const deltaY = Math.abs(this.pos.y - yb) === 1

        const isOneFieldApart = deltaX || deltaY
